# -*- coding: utf-8 -*-
"""
Created on Mon Mar 31 11:46:28 2025
...
if idx_replace == False:
    btf = 1/grp_cvs
win_size_ms = 30 # window size in ms <30ms would match in terms of data we have compared to side decoding, but 50ms for simpicity>
...
n_shuff = 000 # number of times to repeat whole analysis, but shuffling labels
...
skip_first_cycle = True # whether to skip first two movements
###########################################
...
def stim_dir_mean_acc(args):
    ...
    bins_per_500, cycle_skip, y_labs = args
    

    iter_decoding_performance = np.zeros((2, num_windows, grp_cvs)) 
    ...
    for window_idx in range(num_windows):
        ...
        for s in range(2):
            type_win_data = stim_psth_data[s,:,:,:]
            
            start_idxs = [start_idx + m*bins_per_500 for m in range(10)]
            
            rc_data = [
                type_win_data[:,:,start_idxs[m]:start_idxs[m]+window_size] for 
                m in range(0+cycle_skip,10,2)]
            cr_data = [
                type_win_data[:,:,start_idxs[m]:start_idxs[m]+window_size] for 
                m in range(1+cycle_skip,10,2)]
            
            # concatenate data (grp_cvs x n_units x tbins)
            rc_data = np.concatenate(rc_data, axis=-1)
            cr_data = np.concatenate(cr_data, axis=-1)

            for grp in range(grp_cvs):
                
                x_rc = np.hstack(rc_data[np.arange(grp_cvs)!=grp])
                x_cr = np.hstack(cr_data[np.arange(grp_cvs)!=grp])
                X_train = np.hstack([x_rc, x_cr])
                
                #set labels using np.repeat rather than tile
                y_train = np.repeat(y_labs, grp_cvs-1) #-1 because LOO CV method
               
                # Make test data using odd trials
                x_rc_test = rc_data[grp]
                x_cr_test = cr_data[grp]
                X_test = np.hstack([x_rc_test, x_cr_test])
                ...
                iter_decoding_performance[s, window_idx, grp] = score
         
    
    return iter_decoding_performance, boot_idx 


def shuff_stim_dir_mean_acc(args):
    
    boot_idx, avg_psth, df_mid_list, mouse_num_stims, type_names, \
    mouse_ids, nboot, N, grp_cvs, clf, window_size, step_size, t_mids, \
    ntbins, stim_t_mids, stim_idxs, num_windows, num_reg_units, \
    bins_per_500, cycle_skip, all_shuff_y_labs = args
    
    shuf_boot_dec_perf = np.zeros((2, num_windows, grp_cvs, n_shuff))
    ...
    for shuff_idx in range(len(all_shuff_y_labs)):
        iter_args = (
            ...
            num_reg_units, bins_per_500, cycle_skip, 
            all_shuff_y_labs[shuff_idx])
        shuf_boot_dec_perf[:, :, :, shuff_idx], _ = stim_dir_mean_acc(iter_args)
...
if __name__ == "__main__":
    ...
    # to flexibly change svm approach without writing two scripts :)
    if skip_first_cycle == True:
        subdir_prefix = "SKIP2_"
        svm_n_cycles = 4
        cycle_skip = 2 # indexing how many movements to skip
    else:
        subdir_prefix = ""
        svm_n_cycles = 5
        cycle_skip = 0
    
    base_dir = f"./svm_stim_dir_{shuff_nm}_shuffling{zsuffix}/{subdir_prefix}cluster_specific, dt={dt}s, sigma={sigma}/" + \
    ...
    for r, regions in enumerate(region_groups):
        ...
        # Create an SVM model
        clf = svm.SVC(C=C, kernel='linear', max_iter=100000)
        ...
        bins_per_500 = 500//dt_ms
        
        
        #%% stim v stim UNSHUFFLED
        # Prepare storage for decoding performance over time
        unshuffled_decoding_performance = np.zeros((2, num_windows, grp_cvs, 
                                                    nboot))
        ...
        #multi by 5 as 5 + 5 ovement directions
        y_con = np.ones(window_size*svm_n_cycles)  # 1 for one direction
        y_ipsi = np.full(window_size*svm_n_cycles, 2)    # 2 for the other
        ...
        args_list = [(boot_idx, all_avg_psths[boot_idx], df_mid_list, 
                      ...
                      bins_per_500, cycle_skip, y_labs)
        ...
        if n_cpus > 1:
            ...
            with tqdm(total=nboot, position=0, leave=True, ncols=80) as pbar:
                for result, boot_idx in pool.imap_unordered(stim_dir_mean_acc, args_list):
            ...
            with tqdm(total=nboot, position=0, leave=True, ncols=80) as pbar:
                for args in args_list:
                    result, boot_idx = stim_dir_mean_acc(args)
                    unshuffled_decoding_performance[..., boot_idx] = result
                    pbar.update()
        ...
        # Prepare storage for decoding performance over time
        full_shuff_dec_perf = np.zeros((2, num_windows, grp_cvs, n_shuff, nboot))
        
        # prepare all shuffled y labels
        all_shuff_y_labs = [shuff_fn(y_labs) 
        ...
        args_list = [(boot_idx, all_avg_psths[boot_idx], df_mid_list, 
                      ...
                      grp_cvs, clf, window_size, step_size, t_mids, ntbins,
                      stim_t_mids, stim_idxs, num_windows, num_reg_units,
                      bins_per_500, cycle_skip, all_shuff_y_labs)
                     for boot_idx in range(nboot)]
        ...
        with tqdm(total=nboot, position=0, leave=True, ncols=80) as pbar:
            for result, boot_idx in pool.imap_unordered(
                    shuff_stim_dir_mean_acc, args_list):
                # Insert result in the correct index
        ...
        shuffled_decoding_performance = \
            np.mean(full_shuff_dec_perf, axis=(2,4))
